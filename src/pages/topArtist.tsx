...
export default  function TopArtist() {
    ...
    if (currentArtist.isPending || album.isPending || topTracks.isPending) return<div className={"w-full animate-spin flex justify-center"}> <Loader/></div>
    ...
    return (
       ...
       <div className="aspect-square w-300">
           <img className={"aspect-square  rounded-lg object-cover "}
                src={currentArtist?.data?.images?.[0].url} alt={""}/>
            <h1 className="font-extrabold text-2xl pt-5">{currentArtist?.data?.name}</h1>
          <div className="flex gap-10 pt-2 ">
              <div>

                  <h1 className="PlusJakartaSans-Bold text-xl"> {formatFollowers(currentArtist?.data?.followers?.total)}</h1>
                  <p className="PlusJakartaSans-Regular">followers</p>
              </div>
              <div>
                 <h1 className="PlusJakartaSans-Bold text-xl"> {currentArtist?.data?.popularity.toLocaleString()}</h1>
                  <p className="PlusJakartaSans-Regular">Popularity</p>
              </div>
          </div>
       </div>
           <div  className="flex flex-col h-full">
               <div className="relative   w-130 min-h-[200px] bg-white rounded-lg overflow-hidden border">
                   ...
                   <div className="relative overflow-hidden z-10 p-8 flex justify-between  w-full">
                       ...
                       <div>
                           <h2 className="text-2xl font-extrabold tracking-tight">Top Tracks</h2>
                       ...
                       <div className="flex items-center justify-between mb-4">
                           <Drawer  direction={"right"}>
                               <DrawerTrigger><PlayIcon size={50} fill={"#212121"} className={"bg-white rounded-full cursor-pointer p-2"}/></DrawerTrigger>
                               <DrawerContent className={"min-w-xl overflow-auto"}>
                                   <DrawerHeader>
                                       <DrawerTitle>Top Tracks</DrawerTitle>
                                       <DrawerDescription>The most popular and trending songs by this artist</DrawerDescription>
                                   </DrawerHeader>
               ...
               <ScrollArea className={""}>
                   <div className={"flex justify-between w-full  py-4 "}>
                       <div className={" select-none flex gap-2 place-items-center"}>
                           ...
                           <h1 className="PlusJakartaSans-Bold text-2xl">Albums</h1>
                       </div>
